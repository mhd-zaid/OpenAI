...
export default () => ({
    register: async (req, res, next) => {
        try{
            ...
            res.status(201).json({success: true, message: 'Utilisateur crée avec succès !', data: user});
        }catch (error) {
    ...
    login: async (req, res, next) => {
        try {
            ...
            if (!user.isActive) {
                return next(
                    new ValidationError({
                        "global": "Votre compte a été désactivé. Veuillez contacter l'administrateur."
                    })
                );
            }
            ...
            if (!user.isVerified) {
                ...
                content = content
                    ...
                    .replace("{{confirmLink}}", `${process.env.SERVER_URL}/verify/${token}`);
                await sendMail(user.email, "Vérifiez votre compte", null, content);
                return next(
                    new ValidationError({
                        "global": "Votre compte n'a pas été vérifie. Un email de vérification vous a été envoyé"
                    })
                );
            }
            ...
            return res.json({ success: true, message: "Authentification réussie.", data: {token} });
        } catch (error) {
            next(error);
        }
    },
    ...
    resetPassword: async (req, res, next) => {
        try {
            ...
            sendMail(user.email, 'Réinitialisation du mot de passe réussie', null, content);
    ...
    forgotPassword: async (req, res, next) => {
        try {
            ...
            return res.status(200).send({success: true, message: "Un email a été envoyé à l'adresse indiquée."});
        } catch (error) {
            if (error.name === "SequelizeValidationError" || error.name === "SequelizeUniqueConstraintError") {
                error = ValidationError.fromSequelize(error);
            }
    ...
    verifyEmail: async (req, res, next) => {
        try {
            ...
            if (user && user.dataValues.isVerified){
                return res.redirect(`${process.env.CLIENT_URL}/auth/verify`, 200, {success: true, message: 'Votre email a déjà été vérifié. Veuillez vous connecter.', data: user});
            } else if (user){
                await user.update({ isVerified: true });
                return res.redirect(`${process.env.CLIENT_URL}/auth/verify`, 200, {success: true, message: 'Email vérifié avec succès.', data: user});
            } else {
